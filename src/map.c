...
static inline int min_sound_distance_safe(int a, int b) {
    const int INF = 1000000000;               // big sentinel
    int aval = (a < 0) ? INF : a;
    int bval = (b < 0) ? INF : b;
    int m    = (aval < bval) ? aval : bval;
    return (m == INF) ? -1 : m;
}
...
*/
  ...
  int sound_resistance(CHAR_DATA* listener, EXIT_DATA* pexit, ROOM_INDEX_DATA* orig, ROOM_INDEX_DATA* desti) {
    int roomone = 10;
    int roomtwo = 10;
