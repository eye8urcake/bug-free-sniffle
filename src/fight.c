...
*/
    ...
    if (ch->fight_fast == FALSE) {
      ...
      int j = 0;
      if (!IS_NPC(ch)) {
        if ((!str_cmp(arg2, "knockout") || ((IS_FLAG(ch->comm, COMM_SUBDUE)) && !IS_NPC(victim) && !IS_NPC(ch)))) {
          if (guestmonster(ch)) {
      ...
      for (int x = 0; x < DIS_USED; x++) {
        if (discipline_table[x].range > 0) {
          for (int y = 0; y < 10; y += 2) {
            ...
            if (!IS_IMMORTAL(ch) && ch->pcdata->monster_discs[y][i] == discipline_table[x].vnum && discipline_table[x].vnum != DIS_CLAW) {
              send_to_char("You fail to summon your minion, illegal disciplines.\n\r", ch);
              return;
            }
          }
  ...
  bool valid_minion(CHAR_DATA *ch) {
    ...
    for (int i = 0; i < 10; i += 2) {
      if (ch->pcdata->monster_discs[i + 1][x] > 0) {
        ...
        if (!IS_IMMORTAL(ch) && discipline_table[discipline_point(ch->pcdata->monster_discs[i][x])]
            .range > 0 && discipline_table[discipline_point(ch->pcdata->monster_discs[i][x])]
            .vnum != DIS_CLAW)
        return FALSE;
      }
